...
YIMPushManager.bindAccount = function(account){
	...
	var body = new proto.com.baiyu.yim.sdk.web.model.SentBody();
...
YIMPushManager.innerOnMessageReceived = function(e){
	...
	if(type == MESSAGE){
		...
		var data = proto.com.baiyu.yim.sdk.web.model.Message.deserializeBinary(data.subarray(DATA_HEADER_LENGTH,DATA_HEADER_LENGTH+length));
	...
	if(type == REPLYBODY){
		var length = getContentLength(data[1],data[2]);
		var data = proto.com.baiyu.yim.sdk.web.model.ReplyBody.deserializeBinary(data.subarray(DATA_HEADER_LENGTH,DATA_HEADER_LENGTH+length));
		/**
